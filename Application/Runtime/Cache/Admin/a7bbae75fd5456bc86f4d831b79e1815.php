			...
			<div class="am-btn-toolbar">
				...
				<form action="<?php echo U('index') ;?>" method="post">
					<div class="am-btn-group">　
						<select  data-am-selected="{btnSize: 'sm'}"	 name="term_id">
							<?php if(is_array($termlist)): $i = 0; $__LIST__ = $termlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo $vo['id'] ;?>"><?php echo $vo['name'] ;?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
					<div class="am-btn-group">　
						<select  data-am-selected="{btnSize: 'sm'}"	 name="subject_id">
							<?php if(is_array($subjectlist)): $i = 0; $__LIST__ = $subjectlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo $vo['id'] ;?>"><?php echo $vo['name'] ;?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
				...
				<table class="am-table am-table-striped am-table-hover table-main">
					<thead>
						<tr>
							...
							<th class="table-author am-hide-sm-only">分数</th>
							<th class="table-set">操作</th>
						</tr>
					</thead>
					<tbody>
						<?php if(is_array($GradeMess)): $i = 0; $__LIST__ = $GradeMess;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td>
									<input type="checkbox" />
								</td>
								<td><?php echo $vo['id'] ;?></td>
								<td><?php echo getUserName($vo['uid']) ;?></td>
								<td><?php echo getTrueName($vo['uid']) ;?></td>
